...
assert.match(chatBridge, /import \{ adjustChatUnread, isActiveChatThread/, '外部聊天桥接使用统一未读写入口');
assert.match(chatBridge, /await adjustChatUnread\([\s\S]*eventType: 'external-message'/, 'chat_unread_counts 通过统一入口写入成功后才发未读更新');
