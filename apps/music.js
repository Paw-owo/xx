...
export async function mount(containerEl, options = {}) {
  ...
  if (options.page === 'player') state.page = state.currentSongId !== '' ? 'player' : 'library';
  exposeAPI();
  injectStyle();
  render();
}
...
function exposeAPI() {
  const api = state.api || {
    ...
    togglePlay, playNext, playPrevious, playSong,
...
function publicState() {
  ...
  return { songId: song?.id || '', title: song?.title || '', artist: song?.artist || '',
    album: song?.album || '', cover: song?.cover || '', hasSong: Boolean(song),
...
function homePage() {
  ...
  if (current) {
    const card = el('article', 'now-card');
    const body = el('button', 'now-card-body'); body.onclick = () => go('player');
    body.append(cover(current, 'now-cover'), textBlock(playbackLabel(), current.title, artistAlbum(current)));
    const action = iconButton(state.loading?'refresh':state.isPlaying?'pause':'play',state.isPlaying?'暂停':'播放',togglePlay,'now-play');
    card.append(body, action); main.append(card);
  } else {
    const start=el('button','now-card music-start');start.onclick=()=>go('library');
    start.append(cover(null,'now-cover'),textBlock('今天听什么',state.songs.length?'从音乐库选一首开始播放':'导入第一首音乐',state.songs.length?`${state.songs.length} 首音乐在等你`:'本地保存，不会上传'));
    start.append(iconNode('arrow-right',20));main.append(start);
  }
  main.append(homeShortcuts());
  ...
  main.append(songList(sortedSongs('added').slice(0, 4)));
  return main;
}

function homeShortcuts(){
  const wrap=el('div','music-shortcuts');
  const items=[['recent','最近播放',state.songs.filter(s=>s.lastPlayedAt).length,'music'],['favorite','我的收藏',state.songs.filter(s=>s.favorite).length,'heart'],['all','最近添加',state.songs.length,'add'],['playlists','我的歌单',state.playlists.length,'list']];
  items.forEach(([id,label,count,icon])=>{const b=el('button','music-shortcut');b.append(el('span','shortcut-mark'),textBlock('',label,`${count} ${id==='playlists'?'个':'首'}`));b.querySelector('.shortcut-mark').append(iconNode(icon,18));b.onclick=()=>id==='playlists'?go('playlists'):openLibraryFilter(id);wrap.append(b);});
  return wrap;
}
...
function playerPage() {
  ...
  const art=el('div',`player-art${state.isPlaying?' spinning':''}`);art.append(cover(song,'player-cover'));main.append(art);
  const meta=el('div','player-meta');const words=el('div','player-words');words.append(el('h2','',song.title),el('p','',artistAlbum(song)));meta.append(words);
  const fav=iconButton('heart',song.favorite?'取消收藏':'收藏',()=>toggleFavorite(song));fav.classList.add('player-favorite');fav.classList.toggle('selected',song.favorite);meta.append(fav);main.append(meta);
  ...
  const mode=auxiliaryButton('refresh',MODE_LABELS[state.mode],cycleMode,true);
  const lyrics=auxiliaryButton('edit','歌词',()=>{state.lyricsOpen=true;render();});
  const queue=auxiliaryButton('list','队列',openQueue);
...
function auxiliaryButton(icon,label,handler,active=false){const b=el('button',active?'active':'');b.append(iconNode(icon,18),el('span','',label));b.onclick=handler;return b;}
...
function miniPlayer(){const mini=el('aside','music-mini');const song=currentSong();const body=el('button','mini-body');body.onclick=()=>go('player');body.append(cover(song,'mini-cover'),textBlock('',song.title,playbackLabel()));const play=iconButton(state.sourceState==='preparing'?'refresh':state.isPlaying?'pause':'play',state.isPlaying?'暂停':'播放',togglePlay);const next=iconButton('arrow-right','下一首',playNext);mini.append(body,play,next);return mini;}
...
function openSettings(){actionSheet('音乐设置',[['更换播放页背景',()=>uploadBackground('player')],['更换列表背景',()=>uploadBackground('list')],['恢复主题背景',clearBackground],[(state.settings.dualMode?'关闭':'开启')+'一起听',toggleDualMode],['浏览器存储提醒',()=>alert('歌曲保存在这个浏览器里。清理浏览器数据前，请先在设置中导出整机数据备份。')]]);}
...
async function uploadBackground(kind){const input=el('input');input.type='file';input.accept='image/*';input.onchange=async()=>{const file=input.files?.[0];if(!file)return;const value=await readAsDataURL(file);const key=kind==='player'?(state.settings.playerBgKey||'app_bg_music_player'):(state.settings.listBgKey||'app_bg_music_list');if(!await setDB(BLOB_STORE,{key,value,type:file.type,name:file.name})){showToast('背景保存失败');return;}if(kind==='player')state.playerBg=value;else state.listBg=value;render();};input.click();}
...
function sync(){updateProgress();const play=document.querySelector('.player-play');if(play){play.innerHTML='';play.append(iconNode(state.loading?'refresh':state.isPlaying?'pause':'play',26));}document.querySelector('.player-art')?.classList.toggle('spinning',state.isPlaying);if(state.mounted&&state.root&&!document.activeElement?.matches('input,textarea,select'))render();updateDesktopWidget();}
...
function playbackLabel(){const song=currentSong();if(state.sourceState==='preparing')return'正在准备音频';if(state.needsPlayGesture)return'准备好了，点一下继续';if(state.sourceState==='error')return state.error||'播放遇到问题';if(state.isPlaying)return'正在播放 · '+(song?.artist||'未知歌手');return song?.artist||'未知歌手';}
...
.music-app{position:relative;height:100%;overflow:hidden;background:var(--bg-primary);color:var(--text-primary);background-size:cover;background-position:center;font-family:inherit}.music-app:before{content:"";position:absolute;inset:0;background:color-mix(in srgb,var(--bg-primary) 84%,transparent);pointer-events:none}.music-app>*{position:relative}.music-topbar{height:52px;display:flex;align-items:center;justify-content:space-between;padding:0 14px}.music-title{font-size:17px}.icon-btn{display:inline-flex;align-items:center;justify-content:center;width:40px;height:40px;border:0;border-radius:50%;background:transparent;color:inherit}.icon-btn.selected{color:var(--accent)}.icon-btn:active,.music-app button:active{transform:scale(.96)}button:disabled{opacity:.4}.music-nav{display:flex;margin:0 16px 10px;padding:4px;background:var(--surface);border-radius:18px}.music-nav-btn{flex:1;border:0;border-radius:14px;padding:9px;background:transparent;color:var(--text-secondary)}.music-nav-btn.active{background:var(--accent);color:var(--bubble-user-text)}.music-nav-spacer{height:2px}.music-scroll{height:calc(100% - 110px);overflow:auto;padding:8px 16px calc(104px + env(safe-area-inset-bottom));scrollbar-width:none}.music-nav-spacer+.music-scroll{height:calc(100% - 54px)}.section-header{display:flex;align-items:center;justify-content:space-between;margin:18px 2px 10px}.section-header h2{margin:0;font-size:18px}.section-header span,.section-link{color:var(--text-secondary);font-size:12px}.section-link{border:0;background:transparent}.move-up{transform:rotate(180deg)}.move-up:active{transform:rotate(180deg) scale(.96)}.now-card{width:100%;display:flex;align-items:center;gap:12px;padding:12px;border:0;border-radius:24px;background:var(--surface);color:inherit;text-align:left}.now-cover{width:64px;height:64px;border-radius:18px;overflow:hidden;flex:none}.music-text{display:flex;flex-direction:column;min-width:0;flex:1;gap:3px;text-align:left}.music-text strong,.music-text span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.music-text small,.music-text span{color:var(--text-secondary);font-size:12px}.cat-mark{display:flex;align-items:center;justify-content:center;width:100%;height:100%;color:var(--accent)}.cat-mark svg{width:62%;height:62%}img{width:100%;height:100%;object-fit:cover}.song-rail{display:grid;grid-auto-flow:column;grid-auto-columns:108px;gap:10px;overflow:auto}.rail-song{border:0;background:transparent;color:inherit;text-align:left;padding:0}.rail-cover{width:108px;height:108px;border-radius:22px;background:var(--surface);overflow:hidden}.rail-song strong,.rail-song span{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-top:5px}.rail-song span{font-size:12px;color:var(--text-secondary)}.playlist-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.playlist-card{border:0;background:transparent;color:inherit;text-align:left}.playlist-cover{aspect-ratio:1;border-radius:24px;background:var(--surface);overflow:hidden}.playlist-card strong,.playlist-card span{display:block;margin:6px 2px 0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.playlist-card span{font-size:12px;color:var(--text-secondary)}.music-empty{display:flex;flex-direction:column;align-items:center;text-align:center;padding:26px 18px;color:var(--text-secondary)}.music-empty>.cat-mark{width:68px;height:68px;background:var(--surface);border-radius:24px}.music-empty strong{color:var(--text-primary);margin-top:10px}.music-empty p{font-size:13px;margin:6px 0 14px}.music-primary,.music-secondary,.music-danger{border:0;border-radius:18px;padding:12px 16px;font:inherit}.music-primary{background:var(--accent);color:var(--bubble-user-text)}.music-secondary{background:var(--surface);color:var(--text-primary)}.music-danger{background:var(--surface);color:var(--color-danger);margin-top:22px;width:100%}.music-search{display:flex;align-items:center;gap:8px;background:var(--surface);padding:0 13px;border-radius:18px}.music-search input{width:100%;border:0;outline:0;background:transparent;color:inherit;padding:12px 0;font:inherit}.library-tools{display:flex;align-items:flex-start;gap:8px;margin:10px 0}.library-filters{display:flex;gap:6px;overflow:auto;flex:1}.library-filters button,.music-sort{border:0;border-radius:14px;padding:8px 10px;background:var(--surface);color:var(--text-secondary);white-space:nowrap}.library-filters button.active{background:var(--accent);color:var(--bubble-user-text)}.song-list{background:var(--surface);border-radius:24px;overflow:hidden}.song-row{display:flex;align-items:center;padding:5px 7px}.song-row.active{background:color-mix(in srgb,var(--accent) 12%,transparent)}.song-main{display:flex;align-items:center;gap:10px;min-width:0;flex:1;padding:6px;border:0;background:transparent;color:inherit}.song-cover{width:46px;height:46px;border-radius:14px;overflow:hidden;flex:none;background:var(--bg-primary)}.song-duration{font-size:11px;color:var(--text-hint)}.song-row>.icon-btn{width:34px}.music-library>.music-primary{width:100%;margin-top:16px}.import-report{margin-top:12px;padding:14px;border-radius:20px;background:var(--surface)}.import-report p,.import-report small{display:block;color:var(--text-secondary);font-size:12px}.music-groups{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.group-card{display:flex;align-items:center;gap:9px;border:0;border-radius:20px;padding:9px;background:var(--surface);color:inherit}.group-cover{width:52px;height:52px;border-radius:15px;overflow:hidden;flex:none}.playlist-hero{display:flex;align-items:center;gap:14px}.playlist-hero>.playlist-cover{width:100px;flex:none}.playlist-controls{display:flex;gap:8px;margin:18px 0;overflow:auto}.playlist-controls button{white-space:nowrap}.music-player{position:relative;height:calc(100% - 54px);overflow:auto;padding:14px 24px calc(30px + env(safe-area-inset-bottom));text-align:center;background-size:cover;background-position:center}.music-player:before{content:"";position:absolute;inset:0;background:color-mix(in srgb,var(--bg-primary) 74%,transparent)}.music-player>*{position:relative}.player-disc{width:min(65vw,260px);aspect-ratio:1;margin:3vh auto 20px;border-radius:50%;padding:13%;background:var(--text-secondary)}.player-cover{height:100%;border-radius:50%;overflow:hidden;background:var(--surface)}.player-disc.spinning{animation:music-spin 16s linear infinite}.player-meta h2{margin:0;font-size:22px}.player-meta p{margin:6px;color:var(--text-secondary)}.player-favorite{margin:4px auto}.player-progress input,.volume-control input{width:100%;accent-color:var(--accent)}.progress-times{display:flex;justify-content:space-between;font-size:11px;color:var(--text-secondary)}.player-main-controls{display:flex;align-items:center;justify-content:center;gap:22px;margin:18px}.player-main-controls .player-play{width:62px;height:62px;background:var(--accent);color:var(--bubble-user-text)}.player-extras{display:flex;justify-content:center;gap:8px}.player-extras button{border:0;border-radius:14px;padding:9px 12px;background:var(--surface);color:inherit}.volume-control{display:flex;align-items:center;gap:10px;margin:18px auto;max-width:260px}.player-error{padding:12px;border-radius:16px;background:var(--surface);color:var(--text-secondary)}.player-error button{margin-left:7px;border:0;border-radius:12px;padding:7px;background:var(--bg-primary);color:inherit}.music-mini{position:absolute;z-index:5;left:12px;right:12px;bottom:calc(10px + env(safe-area-inset-bottom));display:flex;align-items:center;gap:9px;padding:8px;border:0;border-radius:22px;background:var(--surface);color:inherit;text-align:left}.mini-cover{width:46px;height:46px;border-radius:15px;overflow:hidden;flex:none}.music-mini>.icon-btn{width:36px;flex:none}.music-sheet-backdrop{position:fixed;z-index:10020;inset:0;display:flex;align-items:flex-end;background:color-mix(in srgb,var(--text-primary) 24%,transparent)}.music-sheet{width:100%;max-height:min(72vh,620px);overflow:auto;padding:18px 14px calc(14px + env(safe-area-inset-bottom));border-radius:28px 28px 0 0;background:var(--bg-primary);color:var(--text-primary)}.sheet-title{display:block;padding:4px 10px 12px}.sheet-action,.sheet-cancel{display:block;width:100%;padding:14px;border:0;border-radius:14px;background:transparent;color:inherit;text-align:left}.sheet-action:active{background:var(--surface)}.sheet-cancel{text-align:center;margin-top:7px;background:var(--surface)}.music-lyrics-sheet{position:fixed;z-index:10010;inset:0;display:flex;flex-direction:column;background:var(--bg-primary);color:var(--text-primary)}.music-lyrics-sheet header{height:56px;display:flex;align-items:center;justify-content:space-between;padding:0 16px}.lyrics-body{flex:1;overflow:auto;padding:25vh 22px}.lyric-line{display:block;width:100%;border:0;background:transparent;color:var(--text-secondary);font-size:16px;line-height:1.55;padding:10px;text-align:center}.lyric-line.active{color:var(--accent);font-size:19px;font-weight:600}.music-shortcuts{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;margin:13px 0 6px}.music-shortcut{display:flex;align-items:center;gap:9px;min-width:0;padding:10px;border:0;border-radius:18px;background:color-mix(in srgb,var(--surface) 62%,transparent);color:inherit}.music-shortcut:nth-child(2n){background:color-mix(in srgb,var(--accent) 10%,var(--bg-primary))}.shortcut-mark{display:grid;place-items:center;width:35px;height:35px;flex:none;border-radius:13px;background:color-mix(in srgb,var(--accent) 18%,var(--bg-card));color:var(--accent)}.now-card{position:relative;min-height:116px;padding:10px;background:linear-gradient(135deg,color-mix(in srgb,var(--accent) 25%,var(--bg-card)),color-mix(in srgb,var(--surface) 72%,transparent));border:1px solid color-mix(in srgb,var(--accent) 18%,transparent);overflow:hidden}.now-card:after{content:"";position:absolute;width:96px;height:96px;border-radius:50%;right:-34px;top:-45px;background:color-mix(in srgb,var(--accent) 13%,transparent)}.now-card-body{display:flex;align-items:center;gap:13px;min-width:0;flex:1;padding:0;border:0;background:transparent;color:inherit;text-align:left}.now-cover{width:88px;height:88px;border-radius:25px}.now-play{position:relative;z-index:1;width:46px;height:46px;flex:none;background:var(--accent);color:var(--bubble-user-text)}.section-header{margin-top:20px}.section-header h2{font-size:17px}.song-list{background:transparent;border-radius:0}.song-row{margin:3px 0;padding:4px;border-radius:17px}.song-row:nth-child(odd){background:color-mix(in srgb,var(--surface) 46%,transparent)}.song-row.active{background:color-mix(in srgb,var(--accent) 14%,var(--surface))}.cat-mark{position:relative;background:linear-gradient(145deg,color-mix(in srgb,var(--accent) 24%,var(--bg-card)),color-mix(in srgb,var(--surface) 78%,var(--bg-primary)));overflow:hidden}.cat-mark:before,.cat-mark:after{content:"";position:absolute;border-radius:50%;background:color-mix(in srgb,var(--accent) 12%,transparent)}.cat-mark:before{width:62%;height:62%;left:-22%;bottom:-25%}.cat-mark:after{width:45%;height:45%;right:-18%;top:-14%}.cat-mark svg{position:relative;z-index:1;width:54%;height:54%;filter:drop-shadow(0 4px 8px color-mix(in srgb,var(--accent) 16%,transparent))}.music-mini{padding:8px 9px;background:color-mix(in srgb,var(--bg-card) 86%,transparent);border:1px solid color-mix(in srgb,var(--accent) 18%,transparent);box-shadow:0 10px 28px color-mix(in srgb,var(--text-primary) 12%,transparent);backdrop-filter:blur(18px)}.mini-body{display:flex;align-items:center;gap:9px;min-width:0;flex:1;padding:0;border:0;background:transparent;color:inherit}.mini-cover{width:50px;height:50px;border-radius:17px}.music-scroll{padding-bottom:calc(126px + env(safe-area-inset-bottom))}.music-player{display:flex;flex-direction:column;justify-content:center;min-height:0;padding:10px 24px calc(18px + env(safe-area-inset-bottom))}.music-player:before{background:linear-gradient(180deg,color-mix(in srgb,var(--bg-primary) 58%,transparent),color-mix(in srgb,var(--bg-primary) 82%,transparent))}.player-art{width:min(58vw,220px);aspect-ratio:1;margin:0 auto 18px;padding:7px;border-radius:34%;background:linear-gradient(145deg,color-mix(in srgb,var(--accent) 25%,var(--surface)),color-mix(in srgb,var(--bg-card) 80%,transparent));border:1px solid color-mix(in srgb,var(--accent) 20%,transparent)}.player-cover{height:100%;border-radius:30%;overflow:hidden}.player-art.spinning{animation:music-spin 18s linear infinite}.player-meta{display:flex;align-items:center;gap:10px;text-align:left;margin:0 0 12px}.player-words{min-width:0;flex:1}.player-meta h2,.player-meta p{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.player-meta h2{font-size:23px}.player-favorite{flex:none;background:color-mix(in srgb,var(--surface) 64%,transparent)}.player-progress input{height:30px;margin:0}.player-main-controls{gap:28px;margin:10px 0 12px}.player-main-controls>.icon-btn{width:48px;height:48px;background:color-mix(in srgb,var(--surface) 56%,transparent)}.player-main-controls .player-play{width:66px;height:66px}.player-extras{gap:7px}.player-extras button{display:flex;align-items:center;justify-content:center;gap:6px;min-width:0;flex:1;padding:10px 7px;border-radius:16px;background:color-mix(in srgb,var(--surface) 55%,transparent)}.player-extras button.active{color:var(--accent);background:color-mix(in srgb,var(--accent) 12%,var(--surface))}.volume-control{margin:12px auto 0;color:var(--text-secondary)}.volume-system{font-size:12px}.music-search{border:1px solid color-mix(in srgb,var(--border-soft) 70%,transparent);background:color-mix(in srgb,var(--surface) 64%,transparent)}.playlist-hero{padding:13px;border-radius:24px;background:linear-gradient(135deg,color-mix(in srgb,var(--accent) 14%,var(--surface)),color-mix(in srgb,var(--surface) 40%,transparent))}@media(max-width:350px){.player-art{width:52vw}.player-main-controls{gap:18px}.music-shortcuts{gap:7px}.shortcut-mark{display:none}}@media(max-height:650px){.player-art{width:min(42vw,165px);margin-bottom:10px}.player-meta{margin-bottom:4px}.player-main-controls{margin:5px 0}.volume-control{margin-top:5px}.music-player{justify-content:flex-start}}@keyframes music-spin{to{transform:rotate(360deg)}}@media(max-width:350px){.playlist-grid{grid-template-columns:1fr 1fr}.player-disc{width:58vw}.music-scroll{padding-left:11px;padding-right:11px}}@media(prefers-reduced-motion:reduce){.player-disc.spinning{animation:none}}
