...
function emitMusic(name, extra = {}) {
  const song = currentSong();
  const payload = { source: 'music-player', songId: song?.id || '', title: song?.title || '',
    artist: song?.artist || '', currentTime: state.currentTime, ...extra };
  emit(name, payload);
  emitListeningPlayback(name, payload);
}

function emitListeningPlayback(name, payload = {}) {
  if (!state.settings?.dualMode || !state.settings?.selectedCharacterId) return;
  const playbackState = name === 'music:pause' ? 'paused'
    : name === 'music:change' ? 'changed'
      : name === 'music:ended' ? 'ended'
        : name === 'music:play' ? 'playing' : '';
  if (!playbackState) return;
  emit('music:playback', {
    source: 'music-player',
    characterId: String(state.settings.selectedCharacterId),
    selectedCharacterId: String(state.settings.selectedCharacterId),
...
async function editSong(song){const values=await promptMusicFields('编辑歌曲',[{name:'title',label:'歌曲名',value:song.title,required:true},{name:'artist',label:'歌手（可留空）',value:song.artist},{name:'album',label:'专辑（可留空）',value:song.album}]);if(!values)return;const next={...song,title:values.title.trim(),artist:values.artist.trim(),album:values.album.trim()};if(!await setDB(SONG_STORE,cleanSong(next))){showToast('歌曲信息保存失败');return;}Object.assign(song,next);emitMusic('music:change');render();showToast('歌曲信息已保存');}
...
async function editLyrics(song){const values=await promptMusicFields('编辑歌词',[{name:'lyrics',label:'LRC 时间轴歌词或纯文本歌词',value:lyricsRaw(song),multiline:true}]);if(!values)return;const text=values.lyrics;const synced=/\[\d{1,2}:\d{2}/.test(text);const next={...song,lyricsText:text,lyrics:synced?parseLRC(text):text,lyricsSynced:synced};if(!await setDB(SONG_STORE,cleanSong(next))){showToast('歌词保存失败');return;}Object.assign(song,next);if(sameId(song.id,state.currentSongId))state.lyrics=lyricsFor(song);render();showToast('歌词已保存');}
...
async function deleteSong(song){
  if(!await showConfirm(`要从音乐库删除《${song.title}》吗？音频文件、收藏记录和所有歌单里的引用都会一起移走。`))return;
...
async function createPlaylist(){const values=await promptMusicFields('新建歌单',[{name:'name',label:'歌单名称',value:'',required:true}]);if(!values)return;const name=values.name.trim();const pl=normalizePlaylist({id:generateId('playlist'),name,songIds:[],createdAt:getNow()});if(!await setDB(PLAYLIST_STORE,pl)){showToast('歌单创建失败');return;}state.playlists.push(pl);emit('music:playlist',{action:'create',playlistId:pl.id,name:pl.name,songCount:0});render();}
async function editPlaylist(pl){const values=await promptMusicFields('编辑歌单',[{name:'name',label:'歌单名称',value:pl.name,required:true},{name:'description',label:'歌单简介（可留空）',value:pl.description}]);if(!values)return;const next={...pl,name:values.name.trim(),description:values.description.trim(),updatedAt:getNow()};if(!await setDB(PLAYLIST_STORE,next)){showToast('歌单保存失败');return;}Object.assign(pl,next);emit('music:playlist',{action:'update',playlistId:pl.id,name:pl.name,songCount:pl.songIds.length});render();}
async function deletePlaylist(pl){if(!await showConfirm(`要删除歌单「${pl.name}」吗？只会删除歌单，不会删除音乐库里的歌曲。`))return;if(!await deleteDB(PLAYLIST_STORE,pl.id)){showToast('歌单删除失败');return;}state.playlists=state.playlists.filter(p=>!sameId(p.id,pl.id));emit('music:playlist',{action:'delete',playlistId:pl.id,name:pl.name,songCount:pl.songIds.length});go('playlists');}
...
function promptMusicFields(title,fields){return new Promise(resolve=>{const wrap=el('div','music-form-sheet');wrap.append(el('strong','sheet-title',title));const inputs={};fields.forEach(field=>{const label=el('label','music-form-field');label.append(el('span','',field.label));const input=el(field.multiline?'textarea':'input');if(!field.multiline)input.type='text';input.value=String(field.value||'');input.rows=field.multiline?8:1;inputs[field.name]=input;label.append(input);wrap.append(label);});const actions=el('div','music-form-actions');const cancel=el('button','sheet-cancel','取消');const save=el('button','music-primary','保存');cancel.type='button';save.type='button';cancel.onclick=()=>{hideBottomSheet();resolve(null);};save.onclick=()=>{const values={};for(const field of fields){const value=String(inputs[field.name]?.value||'');if(field.required&&!value.trim()){showToast(`${field.label}要认真填一下`);return;}values[field.name]=value;}hideBottomSheet();resolve(values);};actions.append(cancel,save);wrap.append(actions);showBottomSheet(wrap);});}
