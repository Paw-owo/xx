...
import { createIcon, showToast, showConfirm, showAlert } from '../core/ui.js';
...
async function editSong(song){const title=await inputSheet('歌曲名',{value:song.title,placeholder:'歌曲名',submitLabel:'下一步'});if(title===null||!title.trim())return;const artist=await inputSheet('歌手（可留空）',{value:song.artist,placeholder:'歌手',submitLabel:'下一步'});if(artist===null)return;const album=await inputSheet('专辑（可留空）',{value:song.album,placeholder:'专辑',submitLabel:'保存'});if(album===null)return;const next={...song,title:title.trim(),artist:artist.trim(),album:album.trim()};if(!await setDB(SONG_STORE,cleanSong(next))){showToast('歌曲信息保存失败');return;}Object.assign(song,next);emitMusic('music:change');render();showToast('歌曲信息已保存');}
...
async function editLyrics(song){const existing=lyricsRaw(song);const text=await inputSheet('编辑歌词',{value:existing,placeholder:'输入 LRC 时间轴歌词或纯文本歌词',multiline:true,rows:6,submitLabel:'保存'});if(text===null)return;const synced=/\[\d{1,2}:\d{2}/.test(text);const next={...song,lyricsText:text,lyrics:synced?parseLRC(text):text,lyricsSynced:synced};if(!await setDB(SONG_STORE,cleanSong(next))){showToast('歌词保存失败');return;}Object.assign(song,next);if(sameId(song.id,state.currentSongId))state.lyrics=lyricsFor(song);render();showToast('歌词已保存');}
...
async function deleteSong(song){
  if(!await showConfirm(`确定从音乐库删除《${song.title}》吗？\n音频文件、收藏记录和所有歌单中的引用都会移除。`))return;
...
// 用底部弹层让用户输入文本，替代 window.prompt。
// opts: { value, placeholder, multiline, rows, submitLabel }
// 返回用户输入的字符串；取消则返回 null。
function inputSheet(title,opts={}){document.querySelector('.music-sheet-backdrop')?.remove();const bg=el('div','music-sheet-backdrop');const sheet=el('section','music-sheet');sheet.append(el('strong','sheet-title',title));const field=el(opts?.multiline?'textarea':'input','sheet-input');if(opts?.multiline){field.rows=opts.rows||4;field.style.resize='vertical';}else{field.type='text';}field.value=opts?.value||'';field.placeholder=opts?.placeholder||'';sheet.append(field);const actions=el('div','sheet-input-actions');const cancel=el('button','sheet-cancel','取消');const submit=el('button','sheet-submit',opts?.submitLabel||'确定');submit.classList.add('sheet-submit-primary');let done=false;const finish=value=>{if(done)return;done=true;bg.remove();resolveFn(value);};let resolveFn;const promise=new Promise(resolve=>{resolveFn=resolve;});cancel.onclick=()=>finish(null);submit.onclick=()=>{const v=opts?.multiline?field.value:field.value.trim();finish(opts?.trim===false?field.value:v);};field.addEventListener('keydown',e=>{if(e.key==='Enter'&&!opts?.multiline&&!e.shiftKey){e.preventDefault();submit.click();}});bg.onclick=e=>{if(e.target===bg)finish(null);};actions.append(cancel,submit);sheet.append(actions);bg.append(sheet);document.body.append(bg);requestAnimationFrame(()=>{field.focus();if(opts?.value&&field.select)field.select();});return promise;}
